...
return toBeGraphEquivalent: (util, customEqualityTesters) ->
  compare: (actual, expected) ->
    ...
    compareCustomProperties = (a, b, i, what) =>
      if a == null and b == null
        return true
      if not util.equals(a.propertyDescriptors(), b.propertyDescriptors())
          result.message = -> """
      ...
      for p in a.propertyDescriptors() when p.name != "graph"
        if not util.equals(a[p.name], b[p.name])
          result.message = -> """
